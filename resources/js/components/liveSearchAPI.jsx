    ...
    return (
        <div>
            <div className="relative rounded-md shadow-sm">
                ...
                <input type="text" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-border focus:ring-primary-border sm:text-sm pl-10" placeholder="Rechercher un film" onChange={handleInputChange}/>
            </div>
            {(showResults || query.trim()) &&(<ul className="results-list my-4">
                {results.map(movie => (
                    <li className="flex justify-start bg-gray-50 mb-4 p-4 h-44 max-h-44 rounded-lg transition ease-in-out hover:bg-gray-100" key={movie.id} onClick={() => handleMovieClick(movie.id, movie.title, movie.poster_path, movie.overview)}>
                        <div className="mr-4"><img className="rounded-md" src={"https://image.tmdb.org/t/p/w92" + movie.poster_path} alt="" /></div>
                        <div className="flex flex-col ml-4">
                            <h5>{movie.title}</h5>
                            <p className="text-base">{movie.genre_ids.map(genreId => genreMap[genreId]).join(', ')}</p>
                            <p className="text-base">{renderStars(movie.vote_average)}</p>
                        </div>
                    </li>
                ))}
            </ul>)}
